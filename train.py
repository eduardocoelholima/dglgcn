...
import datasets
from datasets import DblpLikeDataset
...
def main(args):
    ...
    train_mask[data.idx_train.cpu().numpy()] = True
    val_mask = np.zeros(features.shape[0], dtype=bool)
    val_mask[data.idx_val.cpu().numpy()] = True
    test_mask = np.zeros(features.shape[0], dtype=bool)
    test_mask[data.idx_test.cpu().numpy()] = True

    in_feats = features.shape[1]
    n_classes = data.num_labels.sum()
